...
See [`examples/alpine-ajax`](https://github.com/tokio-rs/topcoat/tree/main/examples/alpine-ajax) for this pattern applied to a full todo list, including the non-JavaScript fallback: without Alpine AJAX in the loop, `x-target`'s status-code modifiers and `x-sync` don't apply, since they are purely client-side, so a validation failure there goes through a normal redirect back to the page instead, carrying the message through a flash cookie (see the [cookie guide](crate::cookie)).
